...
// Usage:
// 1) imgaussfilt(I) : Convolve image I with a 2-D gaussian filter of standard deviation 0.5
// 2) imgaussfilt(I,sigma) : Convolve image I with a 2-D gaussian filter of standard deviation sigma

// sigma is a float (default:0.5) , or a 1*2 vector of odd positive scalars signifying the standard deviation in the x and y direction
// signifying the number of rows and columns of the kernels

// 3) imgaussfilt(I,sigma_val,'padding')
...
int opencv_imgaussfilt(char *fname, unsigned long fname_len) {
  SciErr sciErr;
  int intErr = 0;
  int iRows = 0, iCols = 0;
  int *piAddr = NULL;
 ...
 /* **********************************************************************************
  ...
  if (inputarg >= 2) {
    ...
    // Get the standard deviations along x and y
    // 
    sciErr = getMatrixOfDouble(pvApiCtx, piAddr2, &iRows, &iCols, &sigma_val);
    if (sciErr.iErr) {
      sciprint(
          "sigma must be a non-negative scalar or 2-element vector\n");
      return -1;
    }

    if (iCols == 1 && iRows == 1) {
      intErr = getScalarDouble(pvApiCtx, piAddr2, &sigmax);
   ...
   sciprint(
      ...
      sigmax = sigma_val[ 0, 0 ];
      sigmay = sigma_val[ 0, 1 ];
  ...
  int pad_sizex = static_cast<int>((1+2*ceil(2*sigmax)) / 2);
  int pad_sizey = static_cast<int>((1+2*ceil(2*sigmay)) / 2);

  // initializing the box filter kernel used for convolution
  // 
  Size ksize;
  ksize.height= 1+2*ceil(2*sigmax);
  ksize.width= 1+2*ceil(2*sigmay);
  ...
  if (handle_circular_border == 1) {
    ...
    GaussianBlur(tempimg, tempimg,ksize,sigmax,sigmay, BORDER_CONSTANT );
...

  {
    copyMakeBorder(image, tempimg, pad_sizey, pad_sizey, pad_sizex, pad_sizex,
        ...
        GaussianBlur(tempimg, tempimg,ksize,sigmax,sigmay, borderType );
    ...
    GaussianBlur(image, new_image,ksize,sigmax,sigmay, borderType );
  }
